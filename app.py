...
# =========================================================
# Cada producto tiene nombre, categoría, descripción y su
# enlace meli.la generado desde el programa de afiliados.
PRODUCTOS = [
...
def inicio():
    ...
    botones = " ".join(f'<a href="/buscar?q={html.escape(c)}" style="display:inline-block;margin:4px;padding:8px 12px;background:#eee;border-radius:8px;text-decoration:none;color:#222">{html.escape(c.title())}</a>' for c in categorias)
    return f"""
    <!DOCTYPE html><html lang="es"><head><meta charset="UTF-8">
    <meta name="viewport" content="width=device-width,initial-scale=1.0">
    <title>MarketRZ</title></head>
    <body style="font-family:Arial;max-width:950px;margin:40px auto;padding:20px;background:#f5f5f5">
    ...
    <h3>📂 Categorías</h3>{botones}
    <p><a href="/buscar">Ver todos los productos ({len(PRODUCTOS)})</a></p>
...
def login():
    ...
    authorization_url = ("https://auth.mercadolibre.com.ar/authorization?response_type=code"
                         f"&client_id={CLIENT_ID}&redirect_uri={REDIRECT_URI}"
                         f"&code_challenge={challenge}&code_challenge_method=S256")
    return f"<h1>🛒 MarketRZ</h1><p>Redirigiendo a Mercado Libre...</p><a href='{authorization_url}'>🔐 Conectar Mercado Libre</a><script>window.location.href={authorization_url!r};</script>"
...
def buscar():
    ...
    if not resultados:
        return "<h1>🛒 MarketRZ</h1><h2>😕 No encontramos ese producto.</h2><p><a href='/'>🏠 Volver</a></p>"
    tarjetas = ""
    for p in resultados:
        ...
        tarjetas += f'''<div style="background:white;padding:20px;margin:15px 0;border-radius:12px;box-shadow:0 2px 10px #0001"><h2>🛍️ {nombre}</h2><p><strong>Categoría:</strong> {categoria}</p><p>{descripcion}</p><a href="{link}" target="_blank" rel="noopener noreferrer" style="background:#3483fa;color:white;padding:12px 18px;border-radius:8px;text-decoration:none;display:inline-block">🛒 Ver producto en Mercado Libre</a></div>'''
    return f'''<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>MarketRZ</title></head><body style="font-family:Arial;max-width:950px;margin:40px auto;padding:20px;background:#f5f5f5"><h1>🛒 MarketRZ</h1><p>Productos recomendados con tus enlaces de afiliado.</p><form action="/buscar" method="get"><input name="q" value="{html.escape(consulta)}" placeholder="Buscar producto" style="width:65%;padding:12px;border:1px solid #ccc;border-radius:8px"><button type="submit" style="padding:12px 18px;border:0;border-radius:8px">🔎 Buscar</button></form>{tarjetas}<p><a href="/">🏠 Volver al inicio</a></p></body></html>'''
